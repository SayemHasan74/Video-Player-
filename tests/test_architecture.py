...
class ArchitectureTests(unittest.TestCase):
    def test_player_uses_libmpv_render_context_not_native_wid(self) -> None:
        ...
        render_thread = (ROOT / "prism_player/ui/mpv_render_thread.py").read_text(encoding="utf-8")
        ...
        self.assertIn("MpvRenderThread", render_window)
        self.assertIn("QSemaphore", render_thread)
        self.assertIn("self._wake.acquire()", render_thread)
        self.assertIn("context.swapBuffers(self._window)", render_thread)
        self.assertNotIn("makeCurrent", render_window)
        self.assertNotIn("swapBuffers", render_window)
        self.assertIn("createWindowContainer", surface)
        self.assertNotIn("QOpenGLWidget", engine + surface + render_window + render_thread)
